...
class Jobs extends Component {
  ...
  getJobApi=async()=>{
    ...
    if(response.ok===true){
        ...
        this.setState({dataJobs:updatedData,apiStatusJobs:apiStatusContainerJobs.success})
    }else{
  ...
  getJobsSuccess=()=>{
      ...
      const {companyLogoUrl,employmentType,id,jobDescription,location,packagePerAnnum,rating,title}=jobs
      console.log({total, jobs})
      return (
          <>
            {jobs.map(each=>(<Link key={each.id} to={`/jobs/:${id}`}>
  ...
  getProfileApi=async()=>{
    ...
    if(response.ok===true){
        ...
        this.setState({dataProfile:updatedData,apiStatusProfile:apiStatusContainerProfile.success})
    }else{
  ...
  getProfileFailure=()=>(
      <button className="retry-btn" onClick={this.getProfileRetry} type="button">Retry</button>
  )
